<template>
  <transition name="vue-go-top">
    ...
    >
      <div v-if="src !== null" class="vue-go-top__icon">
        <img :src="src" :alt="alt" class="vue-go-top__image" />
        <div :is-active="isRippleActive" :style="rippleStyle" class="vue-go-top__ripple" />
      </div>
      <div v-else class="vue-go-top__content">
        <div :is-active="isRippleActive" :style="rippleStyle" class="vue-go-top__ripple" />
        <slot />
      </div>
    </div>
...
const makeBgImage = (weight, fgColor) => {
  ...
  if (weight === 'bold') {
    return (
      'data:image/svg+xml;charset=utf8, %3Csvg version=%271.1%27 xmlns=%27http://www.w3.org/2000/svg%27 width=%2710px%27 height=%2710px%27%3E %3Cpolygon fill=%27%23' +
      color +
      '%27 stroke=%27none%27 points=%278.646,7.354 5,3.707 1.354,7.354 0.646,6.646 5,2.293 9.354,6.646%27 /%3E %3C/svg%3E'
    )
  }
  return (
    'data:image/svg+xml;charset=utf8,%3Csvg version=%271.1%27 xmlns=%27http://www.w3.org/2000/svg%27 width=%2710px%27 height=%2710px%27%3E %3Cpolygon fill=%27%23' +
    color +
    '%27 stroke=%27none%27 points=%278.589,6.945 5,3.22 1.413,6.945 1.052,6.598 5,2.499 8.948,6.598%27 /%3E %3C/svg%3E'
  )
}
